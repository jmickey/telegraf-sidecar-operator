...
![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![Version: 2.2.0](https://img.shields.io/badge/Version-2.2.0-informational?style=flat-square) ![AppVersion: 0.4.0](https://img.shields.io/badge/AppVersion-0.4.0-informational?style=flat-square)
...
| featureGates | list | `[]` | List of feature gates to enable. Available gates: operator.nativesidecars, telegraf.aggregators, telegraf.processors |
...
| mutatingWebhook.tls.certManager.duration | string | `"8760h"` | Certificate validity duration. |
| mutatingWebhook.tls.certManager.issuerRef | object | `{}` | Existing Issuer/ClusterIssuer (`{name, kind, group}`). Empty = chart creates a self-signed CA. |
| mutatingWebhook.tls.certManager.renewBefore | string | `"720h"` | Renew this long before expiry (must be `< duration`). |
| mutatingWebhook.tls.custom.caBundle | string | `""` | PEM CA bundle for the webhook `caBundle`. |
| mutatingWebhook.tls.custom.cert | string | `""` | PEM server certificate (used when `existingSecret` is empty). |
| mutatingWebhook.tls.custom.existingSecret | string | `""` | Use a pre-existing `kubernetes.io/tls` Secret; takes precedence over `cert`/`key`. |
| mutatingWebhook.tls.custom.key | string | `""` | PEM server private key (used when `existingSecret` is empty). |
| mutatingWebhook.tls.method | string | `"helm"` | Webhook cert provisioning: `helm` (self-signed, default), `certManager`, or `custom`. |
| nameOverride | string | `""` |  |
| nodeSelector | object | `{}` |  |
